...
def letter_filter(bl_list, gl_dict, wordlist):
	...
	for word in wordlist:
		if not any(bltr in word for bltr in bl_list) and not any(word[gl_idx]==gl_val for gl_idx, gl_val in gl_dict.items()) and any(gltr in word for gltr in gl_dict.values()):
	...
	for _ in range(20):
		...
		for i in range(6):
			if i==0:
				...
				nextGuess, wordlist, gl_dict, bl_list, corpos_dict = fn_Wordle_Player(intMatcher, wordlist, gl_dict, bl_list, corpos_dict)
